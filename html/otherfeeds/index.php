...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if(isset($_POST['faSelect'])) {
        if ($_POST['faSelect'] == "Enable") {
            $output = shell_exec('sudo /airplanes/webconfig/helpers/piaware-enable.sh ' . escapeshellarg($_POST['fa-id']));
        }
        if ($_POST['faSelect'] == "Disable") {
            $output = shell_exec('sudo /airplanes/webconfig/helpers/piaware-disable.sh');
        }
    }

    if(isset($_POST['fr24Select'])) {
        if ($_POST['fr24Select'] == "Enable") {
            $output = shell_exec('sudo /airplanes/webconfig/helpers/fr24-install.sh');
            $output = shell_exec('sudo /airplanes/webconfig/helpers/fr24-enable.sh ' . escapeshellarg($_POST['fr24mail']) . ' ' . escapeshellarg($_POST['fr24key']));
        }
        if ($_POST['fr24Select'] == "Disable") {
            $output = shell_exec('sudo /airplanes/webconfig/helpers/fr24-disable.sh');
        }
    }

    if(isset($_POST['fr24Select2'])) {
        if ($_POST['fr24Select2'] == "Reset") {
            $output = shell_exec('sudo /airplanes/webconfig/helpers/fr24-reset.sh');
        }
...
Although this software is published by airplanes.live, we recognize that sometimes users may want to feed data to other sites as well. Our goal is to put the feeders first by making it easy to get the most from their hardware investment.
...
<ul>
<li>airplanes.live was built by aviation enthusiasts, for aviation enthusiasts.
<p>
...
We do not believe in playing these games, and share all data fed to us.
</ul>
    ...
    <br><br>
<?php
$output = shell_exec('sudo /airplanes/webconfig/helpers/piaware-status.sh 2>&1');
echo "<pre>$output</pre>";
?>
    <br><br>
...
?>
    ...
    <br><br>
<?php
$output = shell_exec('sudo /airplanes/webconfig/helpers/fr24-status.sh 2>&1');
echo "<pre>$output</pre>";
?>
    <br><br>
...
<table class="table table-dark"><tr><td>
    ...
    <br><br>
<?php
$output = shell_exec('sudo /airplanes/webconfig/helpers/journal.sh -u piaware.service');
...
?>
    ...
    <br><br>
<?php
$output = shell_exec('sudo /airplanes/webconfig/helpers/journal.sh -u fr24feed.service');
$output = wordwrap( $output, 160);
echo "<pre>$output</pre>";
?>
    </td></tr>
    ...
    <br><br>
<?php
$output = shell_exec('sudo /airplanes/webconfig/helpers/fr24-show-signup-log.sh');
$output = wordwrap( $output, 160);
echo "<pre>$output</pre>";
?>
    </td></tr>
